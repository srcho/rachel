...
| 2026-09-04 | rachel-d5 | 파이널 패스 메모리: 회의 전체 PCM 을 Int16Array 로 펼치던 것(시간당 ≈115MB, Blob 사본까지 2배)을 메타(blob.size→샘플 수)로 계획하고 청크마다 필요한 세그먼트만 IndexedDB 에서 읽어 조립·업로드·해제로 변경. 피크 ≈ 청크 1개(18MB)+WAV 사본 | 아이폰에서 1시간+ 회의 파이널 패스 확인 | `audioStore.getPcm`, `samplesInWav` |
| 2026-09-04 | rachel-d5 | 모바일 UX: (1) 화면 확대 고정 — 원인은 뷰포트 maximumScale 부재 + 입력 글자 12px(iOS 가 포커스 때 확대, 드로어 닫아도 배율 잔존) → `maximumScale:1, userScalable:false` + 터치 기기 폼 컨트롤 16px (2) 할 일을 고정 2×2 그리드(화면 고정, 섹션 내부 스크롤, 카드 컴팩트) (3) FAB 이 마지막 컨트롤을 가리지 않게 모바일 하단 여백. E2E 6/6 | 아이폰 PWA 재확인: 레이첼 드로어 열고 닫은 뒤 배율, 보드 2×2 드래그 | 데스크톱 보드는 4열 그대로 |
| 2026-09-04 | rachel-d5 | 보드 드롭 판정 수정: `closestCorners` 만 쓰면 빈 자리에 놓을 때 이웃 섹션 카드 모서리가 더 가까워 엉뚱한 컬럼으로 판정(2×2 에서 특히) → `pointerWithin` 으로 포인터가 든 섹션 안에서만 고르고, droppable 을 목록 div 에서 섹션 전체로. `scripts/drag-probe.mjs`(모바일 뷰포트 드래그 3회, 새로고침 후 유지) | 아이폰 실드래그 확인 | — |
| 2026-09-04 | rachel-d5 | PC 드롭 결함 2건: (1) 컬럼 자체에 놓으면 항상 맨 끝 → 고스트(DragOverlay) 세로 중심으로 삽입 위치 계산(onDragOver·onDragEnd), 하이라이트는 출발 컬럼이 아닐 때만 (2) `moveCard` 의 자기 자신 판정이 position 문자열 비교라 각 컬럼 첫 카드(`a0`)를 다른 컬럼 첫 카드 앞에 놓으면 무시 → id 비교(테스트 추가). 프로브: 긴 컬럼 위쪽 드롭이 데스크톱·모바일 모두 0번. PC: 레일에 Today 항목, 검색(⌘K) 버튼 | 사용자 확인 | — |
| 2026-09-04 | rachel-d5 | **버그(중대)**: 루트 레이아웃이 스캐폴드부터 ThemeProvider·PwaProvider·Toaster 를 import 만 하고 마운트하지 않았음(git 전체 이력에서 JSX 사용 0회). 영향: 서비스 워커 미등록(푸시 켜기가 `serviceWorker.ready` 에서 영원히 대기), 테마 전환 무효, 토스트 미표시. 레이아웃에 세 프로바이더 마운트 | **사용자**: 설치된 PWA 재실행 후 설정 > 알림 켜기 재시도, 테마 전환 확인 | 기존 E2E 는 셋 다 안 건드려서 못 잡았다 — 토스트 표시를 E2E 에 추가할 것 |
| 2026-09-04 | rachel-d5 | 인사이트 전면 리디자인(ARCHITECTURE §9): lieflat-charts 는 라이브러리가 아니라 HTML 템플릿 스킬 → Basics 갤러리 6종을 React SVG 로 재구현(`core/ui/charts/lieflat`), 데이터 형태별 선택, 결론 제목 계산, 비용은 설정으로(30일 선 + 기능별 tick). 성긴 데이터에 맞춘 적응 크기 | 사용자 확인 | 다크 모드는 테마 변수로 자동 |
