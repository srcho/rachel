...
- **보드**(`full`): 모바일은 고정 2×2 그리드(화면은 움직이지 않고 섹션 안에서 스크롤), 데스크톱은 가로 4열. 헤더 우측 "카드 추가"(`N`) 하나가 진입점 — `NewCardDialog` 에 Google Tasks 항목과 같은 최소 정보(제목·마감·시각·설명)와 보드 고유의 상태·우선순위. 제목의 "내일 3시" 는 마감 제안으로. 컬럼 바닥 인라인 추가는 없앴다. Done 은 오늘(타임존) 완료만 보이고 이전 완료는 개수 + `?done=all` 로 펼친다(에이전트 도구는 항상 전체를 본다). 모바일은 Todo 부터 열린다. 컬럼이 화면 폭을 나눠 갖고(`min-w-64 max-w-sm flex-1`) 높이를 채운다. 카드 목록만 내부 스크롤, 빠른 추가는 컬럼 하단 고정. 모바일은 가로 스냅 스크롤.
- **캘린더**(`full`): 월 = 6행이 뷰포트를 채움(셀당 4개 + `+n`), 주 = 7열 균등·열 내부 스크롤, 일정 = xl 에서 2단(CSS columns). 모든 뷰·Today 위젯·레이첼 컨텍스트는 `calendar/occurrences.ts` 의 `expandOccurrences` 로 이벤트를 **덮는 날마다 조각**으로 펼친다(종일 end 는 배타적이라 1ms 앞 날짜까지; 정렬은 종일 → 이어지는 조각 → 시각순; 라벨 "10:00 →", "계속 · 2/3일", "→ 01:00"). 편집 창은 종일 종료일을 포함일로 보여 주고 저장 때 하루를 더한다.
- **인사이트**(`content`): 패턴 1행 → 처리량 3행 | 회의·일정 2행씩 → 캡처 2행 → 비용 3행(2단). 차트는 패널 높이를 채운다.
- **회의·기억·인박스·설정**(`narrow`): Panel 목록 + Badge. 설정은 섹션마다 Panel.

모바일 배율 고정: 뷰포트 `maximumScale:1, userScalable:false` + `@media (pointer: coarse)` 에서 폼 컨트롤 16px(iOS 포커스 확대 방지).

내비게이션(2026-09-04): 코어가 `Today`·`설정` 항목을 더해 모듈 nav 와 합친다. 모바일은 하단 탭 5개(Today·할 일·일정·회의·설정), 데스크톱 레일은 R(Today)·모듈·맨 아래 설정. 레이첼은 양쪽 모두 **우하단 FAB**(모바일은 탭 위, 데스크톱은 창이 열리면 숨김). 편집 UI(카드·일정·액션 아이템)는 사이드 시트가 아니라 공통 `FormDialog`(컴팩트 패딩, 모바일은 상단 8dvh 정렬). 회의 녹음 화면은 `data-immersive` 로 하단 탭·FAB 을 숨기고 컨트롤 바가 그 자리를 쓴다.
