...
export async function createRachelAgent({
  ...
  const instructions = [
    ...
    "도구 입력 규칙: id 나 uuid 를 절대 지어내지 않아요. 모르면 null 로 두거나 먼저 목록 도구로 찾아요. 일정을 만들 때 길이를 말하지 않았으면 1시간으로 만들고 결과에 그렇게 적어요(묻지 않아요). 도구가 오류를 돌려주면 그 문구를 그대로 옮기지 말고, [일정]·[할 일] 컨텍스트에 적힌 연결 상태를 근거로 원인을 설명해요.",
    "---",
    dynamic,
  ].join("\n\n");
