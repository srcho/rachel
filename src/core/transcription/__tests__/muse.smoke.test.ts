...
const enabled = Boolean(
  process.env.META_MODEL_API_KEY && wavPath && existsSync(wavPath),
);
...
describe.skipIf(!enabled)("Muse smoke (real API)", () => {
  ...
  const wav = enabled
    ? new Blob([readFileSync(wavPath as string)], { type: "audio/wav" })
    : new Blob();
  ...
  it("transcribes Korean with ENDPOINTING and DIARIZATION", async () => {
    ...
    for (const mode of ["ENDPOINTING", "DIARIZATION"] as const) {
      ...
      const r = await p.transcribeFile(
        {
          db,
          userId: "smoke",
          mode,
          feature:
            mode === "DIARIZATION" ? "transcribe_final" : "transcribe_live",
          keywords: ["레이첼", "Muse", "VibeVoice", "김민수"],
          sessionId: `smoke-${mode}-${Date.now()}`,
        },
        wav,
      );
      ...
      for (const t of r.turns)
        console.info(
          `[${(t.startMs / 1000).toFixed(1)}-${(t.endMs / 1000).toFixed(1)}s]${t.speaker ? ` ${t.speaker}:` : ""} ${t.text}`,
        );
