...
## 현재 상태 요약
P2 완료(2026-09-03): 칸반·레이첼 채팅·기억·비용 패널·Google 캘린더(동기화·CRUD·도구)·Today 브리핑이 프로덕션에 있음. 다음 P3 회의(Meta 키 필요). 프로덕션 https://rachel-seven-tau.vercel.app (Vercel `rachel`, icn1) · Supabase `rachel` ref `lpieoftpmhvxibhkhayn`(서울, linked). pg_cron `rachel-jobs`가 1분마다 잡 러너 호출. Meta 키만 미발급. 다음은 PLAN §9 참조(P1 S1.1).
